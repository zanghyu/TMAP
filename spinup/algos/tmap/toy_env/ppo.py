...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    def compute_loss_pi(data):
        ...
        obs, act, adv, logp_old = obs.to(device), act.to(
            device), adv.to(device), logp_old.to(device)
        # Policy loss
    ...
    def test_agent(timestep):
        for j in range(10):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                ta = scale_range(a, -1, 1, env.action_space.low,
                                 env.action_space.high)
                o, r, d, _ = test_env.step(ta)
                ep_ret += r
                ep_len += 1
                this_obs.append(o)
            this_obs = np.array(this_obs)
            np.save('ppo_npy/ppo_obs'+str(timestep) +
                    '_'+str(j)+'.npy', this_obs)
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            if terminal or epoch_ended:
                ...
                if epoch not in save_ep:
                    test_agent(epoch*local_steps_per_epoch+t)
                    # np.save('ppo_npy/obs_'+str(epoch*local_steps_per_epoch+t)+'.npy',this_obs)
                save_ep.append(epoch)
                o, ep_ret, ep_len = env.reset(), 0, 0
