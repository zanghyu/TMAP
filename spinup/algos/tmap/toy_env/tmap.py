...
import gym
import time
import spinup.algos.tmap.core as core
...
class ReplayBuffer:
    """
    A simple FIFO experience replay buffer for TMAP agents.
    """
...
def tmap(env_fn, env_name, exp_name,
         ...
         polyak=0.995, lr=1e-3, lr_alpha=1e-3, lr_beta=1e-3, batch_size=100, start_steps=10000,
         update_after=1000, update_every=50, num_test_episodes=10, max_ep_len=1000,
         logger_kwargs=dict(), save_freq=1, gradient_clipping_norm=1):
    """
    ...
    # def compute_target_entropy(pi_entropy, act_dim):
    #     init_pi_dist = Normal(0, torch.Tensor(0.01*np.ones(act_dim)))
    #     pi_a = init_pi_dist.rsample()
    #     logpi = init_pi_dist.log_prob(pi_a).sum(axis=-1).to(device)
    #     target_entropy = torch.max(0.99*(pi_entropy + logpi)-logpi, -logpi)
    ...
    def compute_loss_q(data, alpha, beta):
        ...
        with torch.no_grad():
            ...
            a2, logp_a2, policy_out_unsquashed, _ = ac.pi(o2)
            # Target Q-values
            q1_pi_targ = ac_targ.q1(o2, a2)
            q2_pi_targ = ac_targ.q2(o2, a2)
            q_pi_targ = torch.min(q1_pi_targ, q2_pi_targ)
            # See Sec.3.2
            backup = r + gamma * (1 - d) * (q_pi_targ - beta * logp_a2)
        ...
        if loss_q > 10000:
            print('alpha: ', alpha)
            print('beta: ', beta)
            print('(alpha+beta) * logp_a2: ', (alpha+beta) * logp_a2)
            print('alpha * logp_a2_old: ', alpha * logp_a2_old)
            print('bkup: ', backup)
    ...
    # Set up function for computing TMAP pi loss
    def compute_loss_pi(data, alpha, beta):
        o = data['obs'].to(device)
        # Target actions come from *current* policy and *old* policy
        pi, logp_pi, pi_bef, _ = ac.pi(o)
        _, logp_pi_old, _, _ = ac.pi_old(
            o, rec_action=pi, rec_action_bef=pi_bef)
        ...
        # See Sec.3.3
        # right_dist = alpha*logp_pi_old.detach() + q_pi
        # diff_ = torch.sub((alpha+beta)*logp_pi, right_dist)
        # zeros_ = torch.zeros_like(diff_)
        # mask_ = torch.gt(diff_, zeros_)
        # print((alpha+beta)*logp_pi)
        # print(right_dist)
        # print(diff_)
        # print(mask_)
        # loss_pi = torch.where(mask_, diff_, zeros_).mean()
        # print(loss_pi)
        loss_pi = ((alpha+beta)*logp_pi -
                   (alpha*logp_pi_old + q_pi)).mean()
        # loss_pi = ((alpha+beta)*logp_pi -
        #           q_pi).mean()
    ...
    def compute_loss_alpha(data, alpha, beta, log_alpha, max_divergence):
        ...
        pi, logp_pi, pi_bef, pi_dist = ac.pi(o)
        _, logp_pi_old, _, pi_old_dist = ac.pi_old(
            o, rec_action=pi, rec_action_bef=pi_bef)

        #print('pi: ',pi.shape)
        # 这里应该是用kl散度来表示，但是由于这里计算出来的kl可能是负的，所以应该去掉小于0的
        # diff_ = torch.sub(logp_pi, logp_pi_old)
        # zeros_ = torch.zeros_like(diff_)
        # mask_ = torch.gt(diff_, zeros_)
        # pi_kl = torch.where(mask_, diff_, zeros_).mean()
        # pi_kl = (logp_pi - logp_pi_old).mean()
        kld = kl_divergence(pi_dist, pi_old_dist).mean()
        # print(kld)
        # loss_alpha = -1.0 * (
        #     log_alpha * (logp_pi - logp_pi_old - max_divergence).detach()).mean()
        loss_alpha = -1.0 * (
            log_alpha * (kld - max_divergence).detach()).mean()
        d = (kld - max_divergence).detach().mean()
        if d > 1:
            print('before train:', log_alpha)
            print('loss_alpha: ', loss_alpha)
            print('d: ', d)
        # d = torch.clamp(d, -max_divergence,5)

        alpha_info = dict(alpha=alpha.detach().cpu().numpy(),
                          pi_kl=kld.detach().cpu().numpy()
                          )

        return loss_alpha, alpha_info, kld

    # See Sec.3.4
    def compute_loss_beta(data, alpha, beta, log_beta, target_entropy):
        o = data['obs'].to(device)
        pi, logp_pi, _, _ = ac.pi(o)
        loss_beta = -1.0 * (
            log_beta * (logp_pi + target_entropy).detach()).mean()

        beta_info = dict(target_entropy=target_entropy,
                         logp_pi=logp_pi.detach().cpu().numpy(),
                         log_beta=log_beta.detach().cpu().numpy()
                         )
    ...
    def update(data, alpha, beta,
        ...
        if not const_dual:
            ...
            loss_alpha, alpha_info, pi_kl = compute_loss_alpha(data, alpha, beta,
                                                               log_alpha, max_divergence)
            loss_alpha.backward(retain_graph=True)
            #torch.nn.utils.clip_grad_norm_(log_alpha, gradient_clipping_norm)
            logger.store(log_alpha_grad=log_alpha.grad)
            logger.store(loss_alpha=loss_alpha)
            logger.store(**alpha_info)
            ...
            # print(log_beta.grad)
            # torch.nn.utils.clip_grad_norm_(log_beta, gradient_clipping_norm)
            # print(log_beta.grad)
            logger.store(log_beta_grad=log_beta.grad)
            logger.store(loss_beta=loss_beta)
            logger.store(**beta_info)
            # print('---------------')
            beta_optimizer.step()
            beta = log_beta.exp()

        return alpha, log_alpha, beta, log_beta, pi_kl

    def get_action(o, deterministic=False):
        return ac.act(torch.as_tensor(o, dtype=torch.float32).to(device),
                      deterministic)

    def test_agent(timestep,md):
        ep_rets = []
        total_obs = []
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                ep_len += 1
                this_obs.append(o)
            this_obs = np.array(this_obs)
            np.save(str(md)+'/obs'+str(timestep)+'_'+str(j)+'.npy',this_obs)
            #total_obs.append(this_obs)
            #ep_rets.append(ep_ret)
            logger.store(TestEpRet=ep_ret, TestEpLen=ep_len)
        #plot_obs = np.array(total_obs[ep_rets.index(max(ep_rets))])
        #np.save('obs'+str(timestep)+'.npy',plot_obs)
    ...
    for t in range(total_steps):
        ...
        if t >= update_after and t % update_every == 0:
            ...
            for j in range(update_every):
                ...
                _alpha, _log_alpha, _beta, _log_beta, pi_kl = update(data=batch, alpha=_alpha, beta=_beta,
                                                                     log_alpha=_log_alpha, max_divergence=_max_divergence,
                                                                     log_beta=_log_beta, target_entropy=_target_entropy)
            #     this_kl += pi_kl
            #     #print('kl: ', pi_kl)
            # this_kl /= update_every
            # print('mean kl: ', this_kl)
            # print('-'*10)
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            test_agent(t+1, _max_divergence)
            ...
            if not const_dual:
                ...
                logger.log_tabular('loss_alpha', average_only=True)
                logger.log_tabular('loss_beta', average_only=True)
                logger.log_tabular('log_alpha_grad', average_only=True)
                logger.log_tabular('log_beta_grad', average_only=True)
                logger.log_tabular('logp_pi', average_only=True)
                logger.log_tabular('target_entropy', average_only=True)
                logger.log_tabular('log_beta', average_only=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='tmap')
    parser.add_argument('--md', type=float, default=0.05)
    ...
    tmap(lambda: gym.make(args.env), args.env, args.exp_name,
